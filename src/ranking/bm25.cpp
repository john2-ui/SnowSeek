...
double bm25(std::uint32_t term_frequency,
    ...
    const double idf = std::log(1.0 +
        (static_cast<double>(document_count) - document_frequency + 0.5) /
        (document_frequency + 0.5));
    const double tf = static_cast<double>(term_frequency);
    const double normalization = k1 *
        (1.0 - b + b * static_cast<double>(document_length) / average_document_length);
