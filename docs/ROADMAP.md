...
- 将内存预算拆分到读取、Token、词典和 Posting 缓冲；
- 达到预算时刷写不可变临时 Segment；
- 实现 K 路归并和临时空间预算；
...
1. 建立构建期内存预算统计与可复现的大语料基线；
2. 按预算刷写临时 Segment，并定义失败清理规则；
3. 实现 K 路归并和临时磁盘空间预算；
4. 接入 `--memory-limit`、`--threads` 与资源档位；
5. 在 AArch64 环境验证峰值 RSS、索引兼容性和查询结果。
