...
当前实现已完成 M3，并完成 M4 的构建内存统计与可复现基线；下一步实现临时
Segment 刷写。
...
完成 M3、M4 内存基线、按预算刷写和单遍 K 路归并后建议依次推进：

1. 增加临时磁盘空间预算和有界 fan-in 多级归并；
2. 接入 `--memory-limit`、`--threads` 与资源档位；
3. 在 AArch64 环境验证峰值 RSS、索引兼容性和查询结果。
