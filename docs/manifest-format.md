...
索引目录中的稳定文件 `MANIFEST` 选择当前可查询 generation。Manifest v1 只允许
一个活动 Segment；多 Segment、Tombstone 和增量变更需要后续版本或后续切片。Segment
文件名不写入 payload，而是由非零 `SegmentId` 推导为十进制、至少 16 位零填充的
`segment-<id>.idx`。

目录没有 `MANIFEST` 时，reader 只允许回退到旧 M4 固定文件
`segment-0000000000000001.idx`。只要 `MANIFEST` 存在，任何 Manifest 损坏、引用缺失
或活动 Segment 校验失败都必须报错，不能静默回退。
...
| 12 | 4 | Flags | `0` |
| 16 | 4 | Header size | `64` |
| 20 | 4 | Active Segment count | `1` |
| 24 | 8 | Generation | 非零，完整发布后单调递增 |
| 32 | 8 | Next SegmentId | 非零且大于所有活动 ID |
| 40 | 8 | Payload length | `count × 8`，v1 为 `8` |
...
Payload 是连续的小端 `u64 SegmentId`。ID 必须非零且严格递增；v1 要求恰好一个。
